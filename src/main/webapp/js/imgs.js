...
function addCart(idd,fidd)
{
	...
	if(add.value=="加入购物车")
	{
		f.action= "/course_design3_war_exploded/cartServlet?key=add&id="+id+"&fid="+fid+"&str=cart";
		f.submit();
	}
	else
		alert("当前商品已加入购物车!");
...
function addStar(idd,fidd)
{
	...
	var f=document.getElementById("fform");
	if(add.value=="关注商品"){
		f.action="/course_design3_war_exploded/cartServlet?key=add&id="+id+"&fid="+fid+"&str=star";
		f.submit();
	}
	else
		alert("当前商品已关注!");
